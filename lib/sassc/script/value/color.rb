...
class SassC::Script::Value::Color < SassC::Script::Value
  ...
  #
  # @raise [SassC::SyntaxError] if any color value isn't in the specified range
  #
  ...
  def with(attrs)
    ...
    SassC::Script::Value::Color.new(attrs, nil, :allow_both_rgb_and_hsl)
  end
  ...
  # @raise [SassC::SyntaxError] if `other` is a number with units
  def plus(other)
  ...
  def piecewise(other, operation)
    ...
    if !other_num && other.alpha != alpha
      raise SassC::SyntaxError.new("Alpha channels must be equal: #{self} #{operation} #{other}")
    end
